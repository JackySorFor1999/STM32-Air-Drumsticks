...
{	
	...
	for (int dx = -LR; dx <= LR; dx++) {
		double ratio = (double)(dx * dx) / (double)(LR * LR);
		double dy = SR * sqrt(1.0 - ratio);

		uint16_t y = usP + (uint16_t)(dy);

		LCD_DrawDot(usC + dx, y, usColor);
	}
	...
	for (int dy = -SR; dy <= SR; dy++) {
		double ratio = (double)(dy * dy) / (double)(SR * SR);
		double dx = LR * sqrt(1.0 - ratio);

		uint16_t x = usC + (uint16_t)(dx);

		LCD_DrawDot(x, usP + dy, usColor);
	}
